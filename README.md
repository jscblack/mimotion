...
- 在 CONFIG 中填写 `TELEGRAM_BOT_TOKEN` 和 `TELEGRAM_CHAT_ID` 后，刷步数完成后会把每个账号的执行结果推送到 Telegram。
- 除此之外，`刷步数` 和 `Random Cron` 两个工作流每次执行结束（无论成功或失败）都会发送一条工作流状态通知（逻辑在 `util/push_util.py` 中，与推送渠道解耦）。`刷步数` 定时触发时，通知里还会带上计划执行时间和实际延迟，方便观察 Actions 调度是否滞后。
- 注意：token 属于敏感信息，只能填写在 GitHub 的 Secret（CONFIG）中，不要提交到仓库。
